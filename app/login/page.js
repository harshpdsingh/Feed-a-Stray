...
export default function Login() {
  ...
  useEffect(() => {
    document.title = "Feed a Stray - Login";
    // Wait for authentication to complete
    if (status === "authenticated" && session) {
      // If the user is the admin → redirect to dashboard
      if (session.user.isAdmin) {
        router.push("/dashboard");
      } else {
        // Normal user → fetch admin to get admin.username
        const fetchAdmin = async () => {
          try {
            ...
            if (res.ok) {
              ...
              if (admin?.username) {
                const adminUsername = encodeURIComponent(admin.username);
                // console.log("Redirecting to admin username:", adminUsername);
                router.push(`/${adminUsername}`);
              } else {
                console.error("⚠️ Admin username missing");
                router.push("/error");
              }
            } else {
  ...
  const handleOAuthLogin = async (provider) => {
    await signIn(provider, { redirect: false }); // Let session redirect
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-md text-white text-center pt-10">
        <div className="mb-6 text-center">
          <h1 className="text-4xl font-bold text-yellow-400">
            🐾 Feed A Stray
          </h1>
          <p className="text-lg mt-2 text-gray-300">
            Every login brings love one paw closer. ❤️
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2 min-h-screen items-center">
          <div className="flex flex-col gap-2 min-h-screen p-10">
            <button
              className="cursor-pointer flex items-center bg-white border border-gray-300 rounded-lg shadow-md max-w-xs px-6 py-2 text-sm font-medium text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
